...
flowchart TD
    ...
    G --> H[PB5/PB7 恢复为下拉输入]
    H --> I[继续识别本次长按，达到门限后打开显示]
```

U1 通过 `s_suppress_s1`、`s_suppress_s1_seen_pressed` 和 200 ms 截止时间过滤唤醒用的这次 S1 事件：按下、短按释放和长按释放不参与泵切换，但同一次按键产生的 `LONG_PRESS` 会保留，因此一次长按即可打开显示。U2 被共享线唤醒时不会自动启动压力泵。
...
共享线唤醒 U1（通常来自 USB）会打开显示；S1 本地唤醒会继续识别当前长按，达到长按门限后打开显示。
