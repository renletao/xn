...
echo === Compiling opt_byte ===

arm-none-eabi-gcc -c %CFLAGS% ../Src/opt_byte.c -o %BUILD_DIR%/opt_byte.o
...
echo === Linking ===
set LDFLAGS=-mcpu=cortex-m0plus -mthumb -specs=nano.specs -Tpy32f002bx5.ld -Wl,-Map=%BUILD_DIR%/Project_opt.map,--cref -Wl,--gc-sections -lc -lm -lnosys

arm-none-eabi-gcc %BUILD_DIR%/opt_byte.o %BUILD_DIR%/system_py32f002b.o %BUILD_DIR%/py32f002b_ll_utils.o %BUILD_DIR%/startup_py32f002bxx.o %LDFLAGS% -o %BUILD_DIR%/Project_opt.elf
if %ERRORLEVEL% neq 0 goto error

arm-none-eabi-objcopy -O ihex %BUILD_DIR%/Project_opt.elf %BUILD_DIR%/Project_opt.hex
arm-none-eabi-objcopy -O binary -S %BUILD_DIR%/Project_opt.elf %BUILD_DIR%/Project_opt.bin

echo === BUILD SUCCESS ===
arm-none-eabi-size %BUILD_DIR%/Project_opt.elf
goto end
